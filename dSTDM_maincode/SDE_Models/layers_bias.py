"""
Common layers code for defining score networks in SDE - pytorch
...
class AttnBlock_noGN(nn.Module):
  """Channel-wise self-attention block without GroupNorm."""
  def __init__(self, channels):
    super().__init__()
    # self.GroupNorm_0 = nn.GroupNorm(num_groups=32, num_channels=channels, eps=1e-6)
    self.NIN_0 = NIN(channels, channels)
...
class ResnetBlockDDPM_noGN(nn.Module):
  """The ResNet Blocks used in DDPM without GroupNorm."""
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, conv_shortcut=False, dropout=0.1):
    super().__init__()
    if out_ch is None:
      out_ch = in_ch
